...
import { 
  ...
  Download, Filter, X, AlertTriangle, Wrench
} from 'lucide-react';
...
export const GarutEmployeeData: React.FC = () => {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter(e => {
      const matchesSearch = (e.nama || '').toLowerCase().includes(search) ||
                            ...
                            (e.perusahaan || '').toLowerCase().includes(search) ||
                            (e.bagian || '').toLowerCase().includes(search) ||
  ...
  const handleExport = () => {
    ...
    const wscols = [
      ...
      { wch: 10 }, { wch: 15 }, { wch: 15 }, { wch: 15 }, { wch: 15 }, { wch: 20 }
    ];
    ws['!cols'] = wscols;

    XLSX.utils.book_append_sheet(wb, ws, "Data Karyawan Garut");
    XLSX.writeFile(wb, `Data_Karyawan_Garut_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured()) {
        setErrorModal({ 
          ...
          message: 'Aplikasi belum terhubung ke database Supabase.' 
        });
        return;
    }
    ...
    try {
      ...
      if (formData.id) {
        const { error } = await supabase.from('data_karyawan_pabrik_garut').update(payload).eq('id', formData.id);
        if (error) throw error;
      } else {
        // Insert
        const { error } = await supabase.from('data_karyawan_pabrik_garut').insert([payload]);
        if (error) {
            if (error.code === '23505') { 
                throw new Error(`Gagal: Kode "${payload.kode}" sudah terdaftar untuk bulan "${payload.bulan}". Duplicate tidak diperbolehkan.`);
            }
            throw error;
        }
      }
      setIsModalOpen(false);
      setSuccessModal({ isOpen: true, title: 'Berhasil', message: 'Data karyawan Garut berhasil disimpan.' });
      fetchEmployees();
    } catch (error: any) {
  ...
  const handleDelete = (id: number) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        const { error } = await supabase.from('data_karyawan_pabrik_garut').delete().eq('id', id);
        if (error) {
  ...
  const handleBulkDelete = () => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        if (!isSupabaseConfigured()) return;
        const { error } = await supabase.from('data_karyawan_pabrik_garut').delete().in('id', selectedIds);
        if (error) {
  ...
  const openModal = (employee?: any) => {
    if (employee) {
      ...
      setFormData({
        ...
        divisi: '', perusahaan: '', bagian: '', bulan: monthYear, keterangan: '', status_aktif: true
      });
    }
    setIsModalOpen(true);
  };
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      ...
      if (!isSupabaseConfigured()) {
          setIsLoading(false);
          setErrorModal({ isOpen: true, title: 'Error', message: 'Database belum terhubung.' });
          if (fileInputRef.current) fileInputRef.current.value = '';
          return;
      }

      try {
        const { data: existingData } = await supabase.from('data_karyawan_pabrik_garut').select('kode, bulan');
        ...
        if (duplicates.length > 0 || internalDuplicates.length > 0) {
             ...
             if (duplicates.length > 0) msg += `❌ ${duplicates.length} data SUDAH ADA di database.\n`;
             if (internalDuplicates.length > 0) msg += `❌ ${internalDuplicates.length} duplikasi GANDA di file Excel.\n`;
             msg += 'Mohon perbaiki data Excel Anda agar tidak ada duplikasi Kode+Bulan.';
        ...
        if (itemsToInsert.length > 0) {
          const { error } = await supabase.from('data_karyawan_pabrik_garut').insert(itemsToInsert);
          if (error) throw error;
          setSuccessModal({ isOpen: true, title: 'Import Selesai', message: `Berhasil mengimport ${itemsToInsert.length} data karyawan Garut.` });
          fetchEmployees();
        } else {
  ...
  const handleDownloadTemplate = () => {
    const template = [
        { 'Kode': 'G001', 'Nama': 'Asep Sunandar', 'P/L': 'L', 'Grade P1': 'A', 'Grade P2': 'Senior', 'Divisi': 'Produksi', 'Perusahaan': 'CV GARUT', 'Bagian': 'Jahit', 'Bulan': 'Oktober 2025', 'Keterangan': 'Tetap' },
        { 'Kode': 'G002', 'Nama': 'Siti Nurhaliza', 'P/L': 'P', 'Grade P1': 'B', 'Grade P2': 'Junior', 'Divisi': 'Packing', 'Perusahaan': 'CV GARUT', 'Bagian': 'Finishing', 'Bulan': 'Oktober 2025', 'Keterangan': 'Kontrak' }
    ];
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(template);
    XLSX.utils.book_append_sheet(wb, ws, "Template");
    XLSX.writeFile(wb, "Template_Karyawan_Garut.xlsx");
  };
              ...
              {isLoading ? (
                ...
                filteredEmployees.map((emp) => (
                  <tr key={emp.id} className={`hover:bg-gray-50 transition-colors group ${selectedIds.includes(emp.id) ? 'bg-pink-50/30' : ''}`}>
                    ...
                    {/* DATA BARU */}
                    <td className="px-6 py-4 text-gray-600">{emp.perusahaan}</td>
                    <td className="px-6 py-4 text-gray-600">{emp.bagian}</td>
                ...
                <tr>
                  <td colSpan={12} className="p-16 text-center text-gray-400 italic">
                    Tidak ada data karyawan Garut. <br/>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10">
              <div>
                <h3 className="font-bold text-xl text-gray-900">{formData.id ? 'Edit Karyawan Garut' : 'Tambah Karyawan Garut'}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input required value={formData.kode} onChange={e => setFormData({...formData, kode: e.target.value})} className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-erp-pink focus:border-erp-pink outline-none transition-all" placeholder="Contoh: G001" />
                ...
                <div>
                  ...
                  <input value={formData.perusahaan} onChange={e => setFormData({...formData, perusahaan: e.target.value})} className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-erp-pink focus:border-erp-pink outline-none transition-all" placeholder="Contoh: CV GARUT" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Bagian</label>
                  <input value={formData.bagian} onChange={e => setFormData({...formData, bagian: e.target.value})} className="w-full border border-gray-300 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-erp-pink focus:border-erp-pink outline-none transition-all" placeholder="Contoh: Jahit / Packing" />
                </div>
