  ...
  return (
    <div className="p-6 space-y-6 bg-erp-bg min-h-full relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-erp-pink mb-2 flex items-center gap-2">
            <Clock className="text-erp-pink" /> Absensi Pabrik
          </h1>
          <p className="text-gray-600 text-sm md:text-lg">Kelola data karyawan dan pencatatan kehadiran harian</p>
        </div>
      </div>
      ...
      <div className="min-h-[500px] animate-fadeIn">
        <ErrorBoundary title="Modul Absensi">
          {activeTab === 'karyawan' && <EmployeeData />}
          {activeTab === 'harian' && <DailyAttendance />}
          {activeTab === 'laporan' && <MonthlyReport />}
        </ErrorBoundary>
      </div>
    </div>
  );
