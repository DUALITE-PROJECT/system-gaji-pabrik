...
import { Wallet, Users, CalendarCheck, FileBarChart, FileText, Database } from 'lucide-react';
import { ErrorBoundary } from '../../Common/ErrorBoundary';
import { AdminMasterSalary } from './AdminMasterSalary';
import { AdminEmployeeData } from './AdminEmployeeData';
...
export const AdminSalaryPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'master' | 'karyawan' | 'harian' | 'laporan' | 'slip'>('master');

  const tabs = [
    { id: 'master', label: 'Master Gaji Admin', icon: Database },
    ...
    { id: 'slip', label: 'Slip Gaji Admin', icon: FileText },
  ];
  ...
  return (
    <div className="p-6 space-y-6 bg-erp-bg min-h-full relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-erp-pink mb-2 flex items-center gap-2">
            <Wallet className="text-erp-pink" /> Gaji Admin (Garut)
          </h1>
          <p className="text-gray-600 text-sm md:text-lg">Manajemen penggajian untuk staff administrasi pabrik</p>
        </div>
      </div>
      ...
      <div className="min-h-[500px] animate-fadeIn">
        <ErrorBoundary title="Modul Gaji Admin">
          {activeTab === 'master' && <AdminMasterSalary />}
          {activeTab === 'karyawan' && <AdminEmployeeData />}
          {activeTab === 'harian' && <AdminDailyAttendance />}
          {activeTab === 'laporan' && <AdminMonthlyReport />}
          {activeTab === 'slip' && <AdminSalarySlip />}
        </ErrorBoundary>
      </div>
    </div>
  );
