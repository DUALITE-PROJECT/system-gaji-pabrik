...
export const StaffSalarySlip: React.FC<StaffSalarySlipProps> = ({ isGarut = false }) => {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      ...
      if (data) {
        // Months
        const months = [...new Set(data.map(d => d.bulan).filter(Boolean))].sort();
        ...
        // Companies
        const companies = [...new Set(data.map(d => d.perusahaan).filter(Boolean))].sort();
        setUniqueCompanies(companies);

        // Divisions
        const divisions = [...new Set(data.map(d => d.divisi).filter(Boolean))].sort();
  ...
  useEffect(() => {
    const fetchSlip = async () => {
      if (employeeCodes.length === 0 || !selectedMonth) {
        setCurrentSlip(null);
        setBoronganData([]);
        return;
      }
      ...
      const currentKode = employeeCodes[currentIndex];

      try {
        // Try Fetch Regular Slip
        let query = supabase
          .from(reportTable)
          .select('*')
          .eq('bulan', selectedMonth)
  ...
  const renderSlip = (data: any, isBulk: boolean = false) => {
    ...
    const tarifPerJam = master?.gaji_per_jam || 0; // Added for Set.H calculation display
  ...
  const renderBoronganSlip = (data: any, isBulk: boolean = false) => {
    ...
    const totalDiterima = Number(data.gaji || 0) + Number(data.bonus || 0);
    const totalBulat = roundUpStepped(totalDiterima);
      ...
      <div key={data.id} className={containerClass}>
        ...
        <div className="mb-2 border-y border-black py-1">
            <div className={`grid grid-cols-4 gap-1 text-center ${fontSizeClass}`}>
                <div><span className="block text-gray-500">Total Hari</span><span className="font-bold">{data.jam_kerja > 0 ? 'Aktif' : '-'}</span></div>
                <div><span className="block text-gray-500">Sakit</span><span className="font-bold">-</span></div>
                <div><span className="block text-gray-500">Izin</span><span className="font-bold">-</span></div>
                <div><span className="block text-gray-500">Lembur</span><span className="font-bold">-</span></div>
            </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
            ...
            <div className="flex items-center gap-2 bg-gray-50 p-1 rounded-lg border border-gray-200">
              <button onClick={handlePrev} className="p-1.5 hover:bg-white rounded-md shadow-sm transition-all" disabled={employeeCodes.length === 0}><ChevronLeft size={18} /></button>
              <span className="text-xs font-medium px-2 min-w-[100px] text-center">{employeeCodes.length > 0 ? `${currentIndex + 1} / ${employeeCodes.length}` : '0'}</span>
              <button onClick={handleNext} className="p-1.5 hover:bg-white rounded-md shadow-sm transition-all" disabled={employeeCodes.length === 0}><ChevronRight size={18} /></button>
            </div>
            ...
            <button onClick={handleBulkPrint} disabled={isBulkPrinting || employeeCodes.length === 0} className="bg-gray-800 text-white px-3 py-2 rounded-lg shadow-md hover:bg-gray-900 transition-colors flex items-center gap-2 text-sm font-medium disabled:opacity-50" title="Cetak Semua Slip (A6)">
