-- =================================================================
-- GARUT AUTO-SYNC SYSTEM - REVISI V11 (FIX UANG BONUS)
-- Update: Formula Uang Bonus dengan Kondisi Bertingkat & Penyesuaian
-- =================================================================
...
BEGIN
    ...
    -- E. Sync Penyesuaian & Data Karyawan (Keluar/Masuk, Keterangan)
    UPDATE public.total_gaji_pabrik_garut t
    SET 
        penyesuaian_bonus = COALESCE(p.penyesuaian_bonus, 0),
        kasbon = COALESCE(p.kasbon, 0)
    FROM public.penyesuaian_gaji_pabrik p
    WHERE t.kode = p.kode AND t.bulan = p.bulan
      AND t.bulan = p_bulan;

    UPDATE public.total_gaji_pabrik_garut t
    SET 
        keluar_masuk = k.keluar_masuk, -- Pastikan kolom ini ada di data_karyawan
    ...
    SET
        ...
        -- 5. UANG BONUS (REVISI V11)
        uang_bonus = CASE 
            WHEN t.periode = 'Periode 1' THEN 0
            ELSE
                (
                    CASE
                        -- Cek Keluar Masuk (STOP if exists)
                        WHEN t.keluar_masuk IS NOT NULL AND t.keluar_masuk NOT IN ('', '-') THEN 0
                        
                        -- Cek Total Absence (STOP if > 0)
                        WHEN (COALESCE(t.i_b,0) + COALESCE(t.i_tb,0) + COALESCE(t.s_b,0) + COALESCE(t.s_tb,0) + COALESCE(t.t_b,0) + COALESCE(t.t_tb,0)) > 0 THEN 0
                        
                        -- Cek Keterangan Libur Pribadi (STOP)
                        WHEN t.keterangan ILIKE '%libur pribadi%' THEN 0
                        
                        -- Cek Libur Perusahaan OR LP > 0 (Half Bonus)
                        WHEN t.keterangan ILIKE '%libur perusahaan%' OR COALESCE(t.lp, 0) > 0 THEN
                            CASE 
                                WHEN t.perusahaan ILIKE '%BORONGAN%' THEN (COALESCE(m.bonus, 0) / 4) / 2
                                ELSE COALESCE(m.bonus, 0) / 2
                            END
                        
                        -- Normal Condition
                        ELSE
                            CASE 
                                WHEN t.perusahaan ILIKE '%BORONGAN%' THEN COALESCE(m.bonus, 0) / 4
                                ELSE COALESCE(m.bonus, 0)
                            END
                    END
                ) - COALESCE(t.penyesuaian_bonus, 0) -- Kurangi Penyesuaian
        END,
...
-- 2. Trigger untuk Master Gaji (Update Bonus Logic)
...
BEGIN
    ...
    SET
        gapok = COALESCE(
            (COALESCE(NEW.gaji_harian, 0) * COALESCE(t.h, 0)) + 
        ...
        u_m = CASE 
            WHEN t.periode = 'Periode 1' THEN 0
            ELSE GREATEST(0, COALESCE(NEW.uang_makan, 0) - (
                ((COALESCE(t.i_tb, 0) + COALESCE(t.s_tb, 0) + COALESCE(t.t_tb, 0)) * 10000) +
                (COALESCE(NEW.uang_makan_harian, 0) * (COALESCE(t.lp, 0) + COALESCE(t.tm, 0) + COALESCE(t.b, 0))) +
                (9000 * COALESCE(t.i_b, 0) + 1000 * COALESCE(t.i_b, 0) * COALESCE(t.i_b, 0)) +
                (9000 * COALESCE(t.s_b, 0) + 1000 * COALESCE(t.s_b, 0) * COALESCE(t.s_b, 0)) +
                (9000 * COALESCE(t.t_b, 0) + 1000 * COALESCE(t.t_b, 0) * COALESCE(t.t_b, 0))
            ))
        ...
        u_k = CASE 
            WHEN t.periode = 'Periode 1' THEN 0
            ELSE GREATEST(0, COALESCE(NEW.uang_kehadiran, 0) - (
                ((COALESCE(t.i_tb, 0) + COALESCE(t.s_tb, 0) + COALESCE(t.t_tb, 0)) * 10000) +
                (COALESCE(NEW.uang_kehadiran_harian, 0) * (COALESCE(t.lp, 0) + COALESCE(t.tm, 0) + COALESCE(t.b, 0))) +
                (9000 * COALESCE(t.i_b, 0) + 1000 * COALESCE(t.i_b, 0) * COALESCE(t.i_b, 0)) +
                (9000 * COALESCE(t.s_b, 0) + 1000 * COALESCE(t.s_b, 0) * COALESCE(t.s_b, 0)) +
                (9000 * COALESCE(t.t_b, 0) + 1000 * COALESCE(t.t_b, 0) * COALESCE(t.t_b, 0))
            ))
        END,
        -- NEW BONUS LOGIC IN TRIGGER
        uang_bonus = CASE 
            WHEN t.periode = 'Periode 1' THEN 0
            ELSE
                (
                    CASE
                        ...
                        WHEN t.keterangan ILIKE '%libur pribadi%' THEN 0
                        WHEN t.keterangan ILIKE '%libur perusahaan%' OR COALESCE(t.lp, 0) > 0 THEN
                            CASE WHEN t.perusahaan ILIKE '%BORONGAN%' THEN (COALESCE(NEW.bonus, 0) / 4) / 2 ELSE COALESCE(NEW.bonus, 0) / 2 END
                        ELSE
                            CASE WHEN t.perusahaan ILIKE '%BORONGAN%' THEN COALESCE(NEW.bonus, 0) / 4 ELSE COALESCE(NEW.bonus, 0) END
                    END
    ...
    UPDATE public.total_gaji_pabrik_garut
    SET hasil_gaji = gapok + gaji_lembur + u_m + u_k + COALESCE(uang_bonus, 0) - COALESCE(kasbon, 0) + COALESCE(penyesuaian_bonus, 0)
    WHERE grade = NEW.grade AND bulan = NEW.bulan;
