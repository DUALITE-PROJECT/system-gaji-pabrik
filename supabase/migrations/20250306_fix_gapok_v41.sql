-- [V41] FIX GAPOK LOGIC: Gapok = (H + LP + TM) * Rate
-- LP dan TM tetap dibayar gapoknya, tapi memotong tunjangan.

CREATE OR REPLACE FUNCTION public.calculate_monthly_report_v41(p_bulan TEXT, p_kode TEXT, p_perusahaan TEXT, p_target_periode TEXT)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
DECLARE
    -- Variabel Metadata
    v_nama TEXT; v_grade_p1 TEXT; v_grade_p2 TEXT; v_divisi TEXT; v_bagian TEXT; v_keluar_masuk TEXT; v_current_grade TEXT;
...
BEGIN
    ...
    -- 1. Ambil Metadata Karyawan (Dari Tabel Garut)
    SELECT nama, grade_p1, grade_p2, divisi, bagian, keterangan INTO v_nama, v_grade_p1, v_grade_p2, v_divisi, v_bagian, v_keluar_masuk
    FROM public.data_karyawan_pabrik_garut WHERE kode = p_kode AND bulan = p_bulan LIMIT 1;

    IF v_nama IS NULL THEN
        SELECT nama, grade_p1, grade_p2, divisi, bagian, keterangan INTO v_nama, v_grade_p1, v_grade_p2, v_divisi, v_bagian, v_keluar_masuk
        FROM public.presensi_harian_pabrik_garut WHERE kode = p_kode AND bulan = p_bulan ORDER BY tanggal DESC LIMIT 1;
    END IF;
    ...
    -- [V41 FIX]: Gapok = (H + LP + TM) * Rate + (Set.H * Rate/2)
    ...
    IF p_target_periode = 'Periode 2' THEN
        SELECT lp, tm INTO v_p1_lp, v_p1_tm FROM public.laporan_bulanan_pabrik_garut WHERE kode = p_kode AND bulan = p_bulan AND periode = 'Periode 1' AND perusahaan = p_perusahaan LIMIT 1;
    ...
    INSERT INTO public.laporan_bulanan_pabrik_garut (
        bulan, periode, perusahaan, kode, nama, grade_p1, grade_p2, divisi, bagian,
    ...
    VALUES (
        p_bulan, p_target_periode, p_perusahaan, p_kode, v_nama, v_grade_p1, v_grade_p2, v_divisi, v_bagian,
        ...
        'Auto-calculated V41', v_keluar_masuk, NOW()
    )
    ON CONFLICT (bulan, periode, kode) DO UPDATE SET
        nama = EXCLUDED.nama, grade_p1 = EXCLUDED.grade_p1, grade_p2 = EXCLUDED.grade_p2, divisi = EXCLUDED.divisi, bagian = EXCLUDED.bagian,
...
GRANT EXECUTE ON FUNCTION public.calculate_monthly_report_v41(TEXT, TEXT, TEXT, TEXT) TO authenticated, service_role;
