...
-- 3. TABEL KARYAWAN PABRIK (Update Struktur Baru)
...
-- Tambahkan kolom jika belum ada (untuk update tabel lama)
DO $$
BEGIN
...
-- 4. TABEL PRESENSI HARIAN (Update Struktur Baru)
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'presensi_harian_pabrik' AND column_name = 'grade_p2') THEN
...
-- 6. PERBAIKAN IZIN AKSES (RLS) - Agar tidak error "Permission Denied"
...
-- Hapus policy lama (jika ada) dan buat baru yang mengizinkan semua akses (untuk kemudahan)
