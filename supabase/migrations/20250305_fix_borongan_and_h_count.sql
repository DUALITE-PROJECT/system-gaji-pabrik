-- [V39] FIX: H Count & Borongan Exclusion

-- 1. Fungsi Cleanup Orphans (Menghapus data laporan yang tidak memiliki presensi)
...
-- 2. Update Fungsi Utama (V39)
CREATE OR REPLACE FUNCTION public.calculate_monthly_report_v39(p_bulan TEXT, p_kode TEXT, p_perusahaan TEXT, p_target_periode TEXT)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    LOOP
        ...
        -- Hitung Agregat Dasar (FIX: H, 1, HADIR)
    ...
    VALUES (
        ...
        'Auto-calculated V39', v_keluar_masuk, NOW()
    )
...
GRANT EXECUTE ON FUNCTION public.calculate_monthly_report_v39(TEXT, TEXT, TEXT, TEXT) TO authenticated, service_role;
