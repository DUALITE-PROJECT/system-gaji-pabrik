-- REVISI LOGIC HITUNGAN TOTAL GAJI (H, LEMBUR, SET.H)
-- Update: 10 Maret 2025

-- 1. Pastikan kolom tipe data mendukung desimal (untuk set_h dan lembur)
ALTER TABLE public.total_gaji_pabrik_garut ALTER COLUMN set_h TYPE NUMERIC;
ALTER TABLE public.total_gaji_pabrik_garut ALTER COLUMN lembur TYPE NUMERIC;

-- 2. Update Function Refresh Total Gaji
...
BEGIN
    ...
    -- C. Update Statistik Kehadiran (REVISED LOGIC)
    UPDATE public.total_gaji_pabrik_garut t
    SET 
        -- 1. Kolom H: Hitung jumlah kehadiran = 'H' (Eksak)
        h = (
            SELECT COUNT(*) 
        ...
        -- 2. Kolom LEMBUR: Sum kolom lembur (jika angka)
        lembur = (
        ...
        -- 3. Kolom SET.H: Sum kehadiran (jika angka) -> Mengcover 0.5, 1, dll
        set_h = (
            SELECT COALESCE(SUM(CASE 
    ...
    -- D. Hitung Nominal Gaji (Standard V41)
    UPDATE public.total_gaji_pabrik_garut t
    SET
        -- Gapok: (H + SET.H + LP + TM) * Rate
        -- Note: set_h sekarang adalah SUM(numeric), jadi sudah merepresentasikan jumlah hari (misal 0.5 + 0.5 = 1)
