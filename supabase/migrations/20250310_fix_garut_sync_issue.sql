-- MEMPERBAIKI FUNGSI SINKRONISASI LAPORAN BULANAN GARUT
-- Masalah: Data ada di Input (Total Gaji) tapi tidak masuk ke Laporan Bulanan

-- 1. Pastikan Kolom Wajib Ada di Tabel Sumber (Total Gaji)
ALTER TABLE public.total_gaji_pabrik_garut ADD COLUMN IF NOT EXISTS keluar_masuk TEXT;
ALTER TABLE public.total_gaji_pabrik_garut ADD COLUMN IF NOT EXISTS keterangan TEXT;

-- 2. Pastikan Kolom Wajib Ada di Tabel Target (Laporan Bulanan)
ALTER TABLE public.laporan_bulanan_pabrik_garut ADD COLUMN IF NOT EXISTS keluar_masuk TEXT;
ALTER TABLE public.laporan_bulanan_pabrik_garut ADD COLUMN IF NOT EXISTS keterangan TEXT;

-- 3. Re-Create Fungsi Sync dengan Penanganan Error yang Lebih Baik
