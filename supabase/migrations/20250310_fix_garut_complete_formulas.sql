-- =================================================================
-- GARUT AUTO-SYNC SYSTEM - COMPLETE FORMULA UPDATE (V10)
-- Mencakup:
-- 1. GAPOK V8.1 (Gaji Harian * H)
-- 2. UANG MAKAN V9 (Potongan Bertahap & Periode Logic)
-- 3. UANG KEHADIRAN V10 (Potongan Bertahap & Periode Logic)
-- =================================================================
...
BEGIN
    ...
    -- E. Hitung Nominal Gaji (ALL COMPONENTS)
    UPDATE public.total_gaji_pabrik_garut t
    SET
        -- 1. GAPOK (Fix V8.1: Use Gaji Harian)
        gapok = COALESCE(
        ...
        u_k = CASE 
            ...
            ELSE GREATEST(0, COALESCE(m.uang_kehadiran, 0) - (
                ...
                -- Progressive Deduction (Bertahap)
                (9000 * COALESCE(t.i_b, 0) + 1000 * COALESCE(t.i_b, 0) * COALESCE(t.i_b, 0)) +
                (9000 * COALESCE(t.s_b, 0) + 1000 * COALESCE(t.s_b, 0) * COALESCE(t.s_b, 0)) +
                (9000 * COALESCE(t.t_b, 0) + 1000 * COALESCE(t.t_b, 0) * COALESCE(t.t_b, 0))
            ))
